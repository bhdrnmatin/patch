...
- `CourtMap` (`app/matches/[id]/_components/`) — a court's static map + مسیریابی link, from
  ...
  Map tiles are Neshan's static API behind `/map/static` — a **route handler**
  (`app/map/static/route.ts`) since 2026-09-21, not a rewrite: Next skips `headers()` for an
  external rewrite, so only a handler can cache what Neshan sends with no cache headers at all. So
...
- Consumed by /matches, /matches/[id], and the parked /tournaments. Feature-local leaves still live in each feature's `_components/`.
...
/(main)/_tournaments   → PARKED 2026-09-22 (out of MVP, no API) — private folder, doesn't route
