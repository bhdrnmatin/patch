...
- [Fix] `BottomSheet` no longer flashes open and shut when a sheet is **mounted** on open rather than toggled via `open` (which `AddPlayerSheet` is). Two causes: the effect depended on `onClose`, whose identity changes every parent render, so it kept tearing down and re-pushing its history entry; and its own `history.back()` is async, so under StrictMode's mount/cleanup/mount the pop landed after the second effect re-pushed the marker and was mistaken for a back press. `onClose` now lives in a ref (deps are `[open]` alone) and a pop only closes the sheet when it leaves the marker behind — which also stops the redundant second `back()` after a real back press.
